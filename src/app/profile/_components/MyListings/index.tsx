    ...
    return (
        <div className="bg-white rounded-lg border border-[var(--color-gray-200)] p-6">
            ...
            {filteredListings.length === 0 && (
                <EmptyState message="No listings found in this category." className="py-12" />
            )}
...
const ListingCard = ({ listing, onViewDetails }: ListingCardProps) => (
    <div className="border border-[var(--color-gray-200)] rounded-lg overflow-hidden">
            ...
            {listing.image ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M3 9L12 2L21 9V20C21 20.5304 20.7893 21.0391 20.4142 21.4142C20.0391 21.7893 19.5304 22 19 22H5C4.46957 22 3.96086 21.7893 3.58579 21.4142C3.21071 21.0391 3 20.5304 3 20V9Z" stroke="var(--color-gray-400)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M9 22V12H15V22" stroke="var(--color-gray-400)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                </div>
            )}
        ...
        <div className="p-4">
            <h4 className="text-sm font-bold text-[var(--color-gray-900)] mb-1 line-clamp-2 uppercase">{listing.title}</h4>
            <p className="text-xs text-[var(--color-gray-500)] mb-2 uppercase">{listing.location}</p>
            <div className="flex gap-2 mb-3">
                {listing.specs.map((spec, index) => (
                    <span key={index} className="text-[10px] text-[var(--color-gray-600)]">
                        {spec}{index < listing.specs.length - 1 && <span className="ml-2">·</span>}
                    </span>
                ))}
            </div>
            <p className="text-xs text-[var(--color-gray-600)] line-clamp-2 mb-4">{listing.description}</p>
            <button
