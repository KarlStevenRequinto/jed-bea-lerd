import React from "react";

interface SearchSvgIconProps {
    color?: string;
    className?: string;
}

const SearchSvgIcon: React.FC<SearchSvgIconProps> = () => {
    return (
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
            <rect width="15" height="15" fill="url(#pattern0_407_1779)" />
            <defs>
                <pattern id="pattern0_407_1779" patternContentUnits="objectBoundingBox" width="1" height="1">
                    <use xlinkHref="#image0_407_1779" transform="scale(0.0111111)" />
                </pattern>
                <image
                    id="image0_407_1779"
                    width="90"
                    height="90"
                    preserveAspectRatio="none"
                    xlinkHref="data:image/png;base64,"
                />
            </defs>
        </svg>
    );
};

export default SearchSvgIcon;
